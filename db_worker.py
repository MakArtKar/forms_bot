...
class DataBase:
    ...
    def get_messages_id_from_question(self, form_id, question_id):
        self.__db_cursor.execute("SELECT message_id FROM QMessages WHERE form_id=? and question_id=? ORDER BY inserted_time", (form_id, question_id))
    ...
    def update_user(self, user: User):
        ...
        self.__db_cursor.execute("UPDATE Users SET state=?, current_form=?, current_question=? WHERE chat_id=?", (user.state, user.current_form, user.current_question, user.chat_id))
        self.commit()
    
    def is_form_id_free(self, form_id):
        self.__db_cursor.execute("SELECT form_id FROM Forms WHERE form_id=?", (form_id,))
        items = self.__db_cursor.fetchall()
        return len(items) == 0
